...
/*   Updated: 2022/11/21 15:41:11 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_philo_routine(t_philo *philo)
{
	...
	while (surveil -> stop != 1)
	{
		...
		{
			if (pthread_mutex_lock(surveil -> done_eat) != 0)
				return (1);
			surveil -> philo_done_eat++;
			if (pthread_mutex_unlock(surveil -> done_eat) != 0)
				return (1);
			return (0);
		}
		else
...
int	ft_finish_philosophers(t_prg *prg)
{
	...
	if (prg -> surveil != 0)
	{
		...
		if (prg -> surveil -> done_eat != 0)
		{
			pthread_mutex_destroy(prg -> surveil -> done_eat);
			free(prg -> surveil -> done_eat);
		}
		free(prg -> surveil);
	}
	// philo 제거 함수 
